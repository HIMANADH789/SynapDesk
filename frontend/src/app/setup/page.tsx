...
export default function SetupPage() {
  ...
  useEffect(() => {
    api.getSetupStatus()
      .then(({ setup_required }) => { if (!setup_required) setAlreadyDone(true); })
      .catch(() => {})
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setError("");
    if (!setupKey.trim()) { setError("Platform setup key is required."); return; }
    if (password !== confirm) { setError("Passwords do not match."); return; }
    if (password.length < 8) { setError("Password must be at least 8 characters."); return; }
    setLoading(true);
    try {
      await api.register(email, password, "platform", "super_admin", setupKey.trim());
      setSuccess(true);
      ...
      <div className="w-full max-w-md rounded-2xl bg-white p-8 shadow-lg">
        <div className="mb-8 text-center">
          ...
          <p className="mt-1 text-sm text-gray-500">
            Create the super admin account. A setup key is required.
          </p>
        </div>
        ...
        {alreadyDone ? (
          <div className="space-y-4 text-center">
            ...
            <Link href="/login" className="inline-block rounded-lg bg-purple-600 px-6 py-2.5 text-sm font-medium text-white hover:bg-purple-700">
              Go to Login
            </Link>
          </div>
        ) : success ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="mb-1 block text-sm font-medium text-gray-700">Platform Setup Key</label>
              <input
                type="password"
                value={setupKey}
                ...
                required
                autoFocus
                placeholder="Enter the PLATFORM_SETUP_KEY from .env"
              ...
              />
              <p className="mt-1 text-xs text-gray-400">Set in backend .env as PLATFORM_SETUP_KEY</p>
            </div>
            ...
            <div className="rounded-lg bg-purple-50 p-3 text-xs text-purple-700">
              This account will have full access to manage all institutions. Keep credentials safe.
              The setup page is only accessible with the correct platform key.
            </div>
